...
model = input('enter model, NFW DC14 or WDM: ')
...
galaxy_name = input("galaxy_name: ")
#galaxy_name = 'KK98-251'
lowmasses = input('low mass? (Y/N)')
...
#The following lines import Hao Fu's AM data intended to be coded in the same style of  KATZ et.al'
...
###############AM TEST: Comment out if unneeded
"""
...
def VgasRadius(galaxy): # to find relationship between Vgas and Radius
    Vgas=np.array([])
    Radius=np.array([])
    for i in range(len(data_lines)):
        temp2 = data_lines[i]
        ...
        if str(temp2[0]) == (galaxy):
            Vgas = np.append([Vgas],float(temp2[5]))
            Radius = np.append([Radius],float(temp2[2]))
    return Vgas, Radius
...
def conc_m_relation_virial(Mgal,Mhalo,M_vir,V_vir):
    #Katz et.al page 4 for concentration-Mass relationship
    
    #V_vir = sprt(GMvir/Rvir)
    #c_vir = Rvir/r_-2
    #alpha = 2.94 -log_10[(10^{x+2.33})^{-1.08}+(10^{x+2.33})^{2.29}]
    #beta = ...
    #gamma = ...
    #r_-2 = ((2-gamma)/(beta - 2))^{1/alpha}r_s

    ####################
    #Seperate Calculation to opline
    ####################
    #Calculate an avg value within 0.3 dex of which will be bounded by opline
    Gravitational_Constant = 6.67408e-11
    Chi  = np.log10(M_gal/M_halo)
    alpha = 2.94 - np.log10((10.0**(Chi+2.33))**(-1.08)+(10**(Chi+2.33))**(2.29))
    beta = 4.23 + Chi*1.34+0.26*Chi**2.0
    gamma = 0.06 - np.log10((10**(Chi+2.56))**(-0.68)+10**(Chi+2.56))
    c_vir = (Gravitational_Constant*M_vir)/(V_vir**2*((2-gamma)/(beta-2))**(1/alpha))
...
#==============================================================================
def opline(galaxy, plot = False, save = False,show=False,bounds=True):
    global wdm_dist
    wdm_dist = np.random.normal(0,0.16)
    #Error Checking########################
    """
    print('Vgas:',len(Vgas(galaxy)),'Vstar:',len(Vstar(galaxy)),'Vtot_data:',
        len(Vtot_data(galaxy)),'Verr:',len(Verr(galaxy)))
    #from this ^^^ line of code comes the error of not being able to broadcast 1d matrixes of differing lengths
    #print(Vtot_data(galaxy))
    print(Vgas(galaxy))
    error_count = 0 # comment out if not testing
    for i in Vgas(galaxy):
        error_count +=1

    print('The Length of Vgas is:',len(Vgas(galaxy)),'The length of Vgas is:',error_count)
    """
    #######################################
    ...
    logMgal.append(np.log10(popt[2]*(Lum(galaxy)*10**9.0)))
    logConc.append(np.log10(popt[1])) 
    logLum.append(np.log10(Lum(galaxy)*10**9.0))
...
def Mhalo_c_relationship(galaxy_name):
    cVals = np.array([])
...
def Mhalo_c_Mstar_scatters(galaxy_names):
    ...
    MLerr = np.array([])
    starmasses = np.array([])
    starluminosities = np.array([])
    centralLums = np.array([])
    chi_array = np.array([])
    theory_mhalo = np.linspace(Mhalo_log_lim[0], Mhalo_log_lim[1], 100) #create theoretical halos between the limits
    ...
    for i in range(len(galaxy_names)):
        ...
        varis = opline(galaxy_name, False) # uncheck to true to plot galaxy rotation curves
        popt, pcov = varis[0], varis[2] #coefficients and covariances from curve fit
...
#        print(galaxy_name, i[0], acc_obs(r_list(galaxy)))
        ...
        starluminosities = np.append(starluminosities, Lum(galaxy_name)/(10.**9.))
        #print(starmasses[-1])
        centralLums = np.append(centralLums, Bright(galaxy_name))
        if model == 'NFW' or model == 'DC14':
    ...
    #d[-1][0].set_linestyle(':') # if everything breaks uncomment this, guesswork at best oh god help me
    #d[-1][1].set_linestyle(':') # LOOK HERE ^&^^^^^^^^^^^^^
    clb = plt.colorbar()
    clb.set_label("$log_{10}(Central\ surface\ brightness)\ (L_{\odot}\ pc^{-2})$")
    plt.show()
    ...
    #plt.savefig(model + " $M_{halo} - c$ relation") # THIS DOESNT WORK FOR SOME ARBITRATY STUPID REASON
    "Mhalo-Mstar relation"
    plt.figure(2)
    plt.title("$M_{star} M_{halo} relationship$")
    plt.scatter(masses, starmasses, c=centralLums, cmap='jet', vmax = 4.4) #LOOK HERE FOR STARTING POINT DATA MASSES HALO/GALAXY
    d = plt.errorbar(masses, starmasses, xerr = Herr, yerr = MLerr, fmt='none', ecolor='0.7', elinewidth='0.5', marker=None, mew=0, linestyle = ':')
    ####################################
    #Writing Data to file 

    file = open(str(model)+'_MstarMhalo.dat','w')
    file.write(str(starmasses)+'\n'+str(masses))
    file.close()

    file = open(str(model)+'_concentration.dat','a')
    for i in range(len(masses)):
        file.write(str(masses[i])+' '+str(cVals[i])+' '+str(centralLums[i])+'\n')
    file.close()

    file = open(str(model)+'_MLratio.dat','a')
    for i in range(len(starmasses)):
        file.write(str(starmasses[i])+' '+str(starluminosities[i])+'\n')
    file.close()
...
if Do_stats == 'Y':
    ...
    cdf = np.linspace(0, 1, num=len(galaxy_names))
...
def targeted_sampling(model,galaxy_name,random=False,targeted=True,N=10,Model_DC14=True,Model_NFW=True):

    #selects galaxy names at random from listed galaxies then feeds its into plotting loop
    #print(galaxy_names_plot)
    if random == True:

        for i in galaxy_names_plot:
            i = galaxy_name

            if model==DC14:
                
                
                #galaxy_name = input("Galaxy ID: ")

                opline(galaxy_name) # do not under any circumstances make this a list, array or 
                #anything plural, it will return a truth error. 
...
def plotSPARCdata(data):



    Mhalo = [optimal(y)[0] for y in [x for x in data]]
    ML = [optimal(y)[2] for y in [x for x in data]]
    Luminosities =[Lum(x) for x in data]
    Mgal = [ML[i]*Luminosities[i] for i in range(len(ML))]

    plt.figure(figsize=(12,12),dpi=60)
    plt.scatter(Mgal,Mhalo)
    plt.savefig()
    plt.show('SPARC galaxies'+_model)
    return

def returnallgalaxies():
    plt.figure(figsize=(12,12),dpi=60)

    for i in wholesample(data_lines1):
        galaxy_name = i 
        temps = opline(galaxy_name)

    return logMhalo , logMgal

def plotrelations(logMgal):
    bulgedata = []
    gasdata = []
    stardata = []
    ...
    for i in wholesample(data_lines1):
        #opline(i)
        bulgedata.append(Vbulge(i))
        gasdata.append(Vgas(i))
        stardata.append(Vstar(i))

    #print(type(bulgedata),type(gasdata),type(stardata))
    #print(bulgedata,gasdata,stardata)
    """
    plt.plot(logMgal,bulgedata)
    plt.show()
    plt.plot(logMgal,gasdata)
    plt.show()
    plt.plot(logMgal,stardata)

    plt.plot(10**logMgal,bulgedata)
    plt.show()
    plt.plot(10**logMgal,gasdata)
    plt.show()
    plt.plot(10**logMgal,stardata)"""
    return


if input('plot relations (Y/N)?') == 'Y':
    plt.figure(figsize=(12,12),dpi=60)
    for i in wholesample(data_lines1):
        galaxy_name = i 
        opline(i)
    ...
    popt, _ = op.curve_fit(func, logLum, logMgal)
    a, b = popt
    x_line = np.arange(min(logLum), max(logLum), 1)
    y_line = func(x_line, a, b)
    plt.plot(x_line, y_line, '--', color='red')
    plt.xlabel("Log_{10}Luminosity")
    plt.ylabel("Log_{10}Mass")
    plt.scatter(logLum,logMgal)
    plt.show()
    print(a,b,(max(logMgal)-min(logMgal))/(max(logLum)-min(logLum)))

    plt.figure(figsize=(12,12),dpi=60)
    for i in wholesample(data_lines1):
        galaxy_name = i 
        Vgas, Vradus = VgasRadius(i)
        plt.plot(Vradus,Vgas,label=str(galaxy_name))
    plt.legend()
    plt.show()
    ...
    for i in wholesample(data_lines1):
        galaxy_name = i 
        Vstar, Vradus = VstarRadius(i)
        plt.plot(Vradus,Vstar,label=str(galaxy_name))
    plt.legend()
    plt.show()
    ...
    for i in wholesample(data_lines1):
        galaxy_name = i 
        Vbulge, Vradus = VgasRadius(i)
        plt.plot(Vradus,Vbulge,label=str(galaxy_name))
    plt.legend()
    plt.show()

galaxy_names_plot = [galaxy_names[i] for i in [x for x in random.sample(range(0,len(galaxy_names)),10)]]


logMhalos = []
logMGal = []
#plotSPARCdata(wholesample(data_lines1))
#THE LINES BELOW PLOT THE ENTIRE SET FOR A SPECIFIED MODEL
if input("plot sparc data (Y/N") == 'Y':
    plt.figure(figsize=(12,12),dpi=60)
    ...
    error = 0.3
    sigma2 = error*2
    AM_HaoFu2020_galaxies= np.asarray(AM_HaoFu2020_galaxies)
    AM_HaoFu2020_Halos = np.asarray(AM_HaoFu2020_Halos)
    plt.scatter(logMhalo,logMgal)
    plt.plot(AM_HaoFu2020_Halos,AM_HaoFu2020_galaxies)
    ...
    plt.scatter(logMhalo,logConc)
    plt.title('SPARC DATASET - concentration '+model)
    plt.ylabel('$log_{10}[C_{Vir}]$')
    plt.xlabel('$log_{10}[M_{Halo}/M_{\odot}]$')
    plt.savefig('SPARC DATASET-conc'+model+'_bounded')
    plt.show()

    sns.regplot(x=logMhalo, y=logConc);
    plt.title('SPARC DATASET - concentration '+model)
    plt.ylabel('$log_{10}[C_{Vir}]$')
    plt.xlabel('$log_{10}[M_{Halo}/M_{\odot}]$')
    plt.savefig('SPARC DATASET-conc'+model+'_SB'+'_bounded')
    plt.show()

    if lowmasses == 'Y':
        lowgaldata, lowhalodata = zip(*sorted(zip(logMgal, logMhalo)))
        lowgaldata = lowgaldata[:20]
        lowhalodata = lowhalodata[:20]
        plt.scatter(lowhalodata,lowgaldata)
        plt.plot(AM_HaoFu2020_Halos,AM_HaoFu2020_galaxies)
        plt.title('SPARC DATASET - AM - '+model+'lowmass')
        plt.ylabel('$log_{10}[M_{*}/M_{\odot}]$')
        plt.fill_between(AM_HaoFu2020_Halos,AM_HaoFu2020_galaxies+error,AM_HaoFu2020_galaxies-error,alpha=0.5)
        plt.fill_between(AM_HaoFu2020_Halos,AM_HaoFu2020_galaxies+sigma2,AM_HaoFu2020_galaxies-sigma2,alpha=0.5)
        plt.xlabel('$log_{10}[M_{Halo}/M_{\odot}]$')
        plt.savefig('SPARC DATASET-AM-'+model+"_error_"+'point2dex'+'_bounded'+'lowmass')
        plt.show()


if input("plotrelations? (Y/N)") == "Y":
    plotrelations()


Model_MCMC = input("model MCMC? (Y/N): ")
if Model_MCMC == "Y":
